    ...
    like() {
      if (this.num % 2 === 0) {
        this.$api
          ...
          .then((res) => {
            if (res.code === 200) {
              let Collectiongood=new array()
             Collectiongood.push(this.goods.goodsOne);
             console.log(Collectiongood)
              localStorage.setItem("Collectiongood", JSON.stringify(Collectiongood));
            }
            // console.log(res);
          })
          .catch((err) => {
        ...
        this.$api
          ...
          .then((res1) => {
            if (res1.code === 200) {
              console.log(res1);
              localStorage.removeItem("Collectiongood");
            }
          })
          .catch((err) => {
            console.log(err);
