                ...
                this.$api
                  ...
                  .then((res) => {
                    if (res.code === 200) {
                      ...
                      if (localStorage.this.user.nickname+"collection") {
                        let collection = JSON.parse(
                          localStorage.getItem(this.user.nickname+"collection")
                        );
                        collection.map(item=>{
                          if(item.name===this.user.nickname){
                            item.good.push(this.goods.goodsOne);
                          }
                          else{
                        ...
                        collection[0].good.push(this.goods.goodsOne);
                        localStorage.setItem(
                          this.user.nickname+"collection",
                          JSON.stringify(collection)
                        );
                      }
                    }
            ...
            .then((res1) => {
              if (res1.code === 200) {
                this.$api
                  ...
                  let collection = JSON.parse(localStorage.getItem(this.user.nickname+"collection"));
                collection.map((item) => {
                  if (item.name===this.user.nickname) {
                    item.good.map((item1,index)=>{
                      if(item1.id===this.goods.goodsOne.id){
                        console.log(item1)
                         item.good.splice(index,1)
                      }
                    })
                  }
                });
                localStorage.setItem(this.user.nickname+"collection", JSON.stringify(collection));
