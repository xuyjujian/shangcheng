...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      console.log(this.goodslist[0].count,"aaa");
      this.$api.postOrder({
         address:this.address.address,
        tel:this.address.tel, // 电话
        orderId:this.orderId, // 所有商品的id(数组)
        totalPrice:this.num/100, // 总价格
        idDirect:this.goodslist[0].idDirect, // 用来判断是购物车结算还是直接购买(直接购买为true, 购物车结算为false)
        count:this.goodslist[0].count // 商品数量
      }).then(res=>{
        if(res.code===200){
