  ...
  <div class="box">
    ...
    <div class="box1">
      ...
      <van-form @submit="onSubmit">
        ...
        />
        <van-field name="手机号码" label="手机号码" placeholder="仅注册用" />
        <div class="df jc_b">
          <van-field name="短信验证码" label="短信验证码" placeholder="仅注册用"></van-field>
          <div class="mr">
            <van-button class="mas" type="primary" size="small" @click="message">发送验证码</van-button>
          </div>
        </div>
        <div class="df jc_b">
          <div>
...
export default {
  ...
  methods: {
    message() {
      this.mas.disable = true;
      let time = 30;
      let timer = setInterval(() => {
        time--;
        mas.innerText=`${time}s后重新发送`
        if(time===0){
          mas.disabled = false
          mas.innerText = '重新发送'
          // 清除定时器
