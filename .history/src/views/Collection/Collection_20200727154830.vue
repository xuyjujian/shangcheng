...
export default {
      ...
      this.$api
        ...
        .then((res) => {
          if (res.code === 200) {
            ...
            localStorage.setItem(this.username, JSON.stringify(this.collection));
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  mounted() {
    ...
    this.username=this.user.nickname+"collection"
    this.collection = JSON.parse(localStorage.getItem(this.username));
    console.log(this.collection)
