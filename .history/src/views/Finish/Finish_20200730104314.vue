<template>
  <div>
    <div class="wt">
      <div class="icon" @click="icon">
        <van-icon name="arrow-left" color="#1989fa" size="20" />
      </div>
      <div class="pt pb fsl tex" style="border-bottom: #EEEEEE solid 1px">我的订单</div>
    </div>
    <van-tabs v-model="active">
      <van-tab title="全部">
        <div style="background:#F2F2F2">
          <div v-for="(item,index) in list" :key="index" class="df jc_c ai_c wt mt mb">
            <div class="box mt">
              <div class="mt mb" style="border-bottom: #EEEEEE solid 1px ">订单编号:{{item.order_id}}</div>
              <div v-for="(item1,index1) in item.order_list" :key="index1" class="df jc_b" style="border-bottom: #EEEEEE solid 1px ">   
                <div>
                  <img :src="item1.image_path" alt="">
                </div>
                <div>{{item1.name}}</div>
                <div>
                  <div>￥{{item1.present_price}}</div>
                  <div>X{{item1.count}}</div>
                </div>
              </div>
              <div class="df fd_c jc_e ai_e" style="border-bottom: #EEEEEE solid 1px ">
                <div>创建时间：{{item.add_time}}</div>
                <div>收货地址：{{item.address}}</div>
                <div>共计{{item.order_list.length}}件商品 合计：{{item.mallPrice}}</div>
              </div>
            </div>
          </div>
        </div>
      </van-tab>
      <van-tab title="待支付"></van-tab>
      <van-tab title="待发货"></van-tab>
      <van-tab title="待收货"></van-tab>
      <van-tab title="已完成">
        <div style="background:#F2F2F2">
          <div v-for="(item,index) in list" :key="index" class="df jc_c ai_c wt mt mb">
            <div class="box mt">
              <div class="mt mb" style="border-bottom: #EEEEEE solid 1px ">订单编号:{{item.order_id}}</div>
              <div v-for="(item1,index1) in item.order_list" :key="index1" class="df jc_b" style="border-bottom: #EEEEEE solid 1px ">   
                <div>
                  <img :src="item1.image_path" alt="">
                </div>
                <div>{{item1.name}}</div>
                <div>
                  <div>￥{{item1.present_price}}</div>
                  <div>X{{item1.count}}</div>
                </div>
              </div>
              <div class="df fd_c jc_e ai_e" style="border-bottom: #EEEEEE solid 1px ">
                <div>创建时间：{{item.add_time}}</div>
                <div>收货地址：{{item.address}}</div>
                <div>共计{{item.order_list.length}}件商品 合计：{{item.mallPrice}}</div>
              </div>
            </div>
          </div>
        </div>
      </van-tab>
    </van-tabs>
  </div>
</template>

<script>
export default {
  name: "",
  props: {},
  components: {},
  data() {
    return {
      active: null,
      list: [],
    };
  },
  methods: {
    icon() {
      this.$router.go(-1);
    },
    getMyOrder() {
      this.$api
        .getMyOrder()
        .then((res) => {
          if (res.code === 200) {
            this.list = res.list;
          }
          console.log(this.list);
        })
        .catch((err) => {
          console.log(err);
        });
    },
  },
  mounted() {
    this.getMyOrder();
    this.active = this.$route.query.num;
  },
  watch: {},
  computed: {},
};
</script>

<style scoped lang='scss'>
.icon {
  position: absolute;
  top: 10px;
  left: 10px;
}
.box{
  width: 90%;
  background: white;
  padding: 10px;
}
img {
  width: 100px;
}
</style>