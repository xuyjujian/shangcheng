  ...
  methods: {
    ...
    search1() {
      for (let i in this.cities) {
          if(i.toLocaleUpperCase()===this.value||i.toLocaleLowerCase()===this.value){
      ...
      arr.map(item=>{
          this.search=item.filter(item1=>{
              console.log(item1.name,"qwe")
              return JSON.stringify(item1.name).indexOf(this.value) > -1
          })
      })
